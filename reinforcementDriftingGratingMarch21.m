...
%% determine the day number of reinforcement

sessionID_reinforcement = find(trainingStage == "Reinforcement");

sessionID_reinforcementToInclude = []; %to exclude the extra sessions that should be ignored in

for sessionCounter=1:length(sessionID_reinforcement)
    
    sessionInd = sessionID_reinforcement(sessionCounter);
    ...
    if sessionInd == sessionsIndInThisDay(end) % just include the last session from this day
        sessionID_reinforcementToInclude = [sessionID_reinforcementToInclude sessionInd];
    end
end
    
todayDayNoOfReinforcement = length(sessionID_reinforcementToInclude);
...
sgtitle({strcat('#',mouseNumber, ' Day', num2str(todayDayNoOfReinforcement)) ,string(date),...
...
fileName = strcat('allLicksDist_Day', num2str(todayDayNoOfReinforcement));
...
fileName = strcat('firstLicksDist_Day', num2str(todayDayNoOfReinforcement));
fileAdd = strcat(googleDriveLocalFolderAddress,'\',mouseNumber,'\',fileName);
print(h,fileAdd,'-dpdf','-r0')
...
xlim([0, todayDayNoOfReinforcement+1])

xticks(1:todayDayNoOfReinforcement)
...
fileName = 'PerformanceAcrossDays_Reinforcement';
fileAdd = strcat(googleDriveLocalFolderAddress,'\',mouseNumber,'\',fileName);
print(h,fileAdd,'-dpdf','-r0')
...
fileName = 'ResponseDelayAcrossDays_Reinforcement';
fileAdd = strcat(googleDriveLocalFolderAddress,'\',mouseNumber,'\',fileName);
print(h,fileAdd,'-dpdf','-r0')
